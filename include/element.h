...
public:
    ...
    Element(Index id,std::vector<Nodeptr> nodes):m_id(id),m_nodes(nodes),
    m_edges{},m_isBoundary(false),m_init(false) {}
    virtual ~Element();


//    virtual functions
    // virtual void getBsmat_at(double x, double y, Oceane::Matrix& bsmat, double& detj);
    // virtual void getBdmat_at(double x, double y, Oceane::Matrix& bdmat, double& detj);
    ...
    //void getBmats_at(double x, double y, Oceane::Matrix& bdmat,
                             //Oceane::Matrix& bsmat, double& detJ);
    //Add node in element
